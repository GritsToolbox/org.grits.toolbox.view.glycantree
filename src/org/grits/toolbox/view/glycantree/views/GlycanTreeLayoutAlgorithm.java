 ...
 * 
 * @author ilmta, @modified Sena Arpinar To have a horizontal tree layout instead of the original vertical tree
 */
public class GlycanTreeLayoutAlgorithm extends AbstractLayoutAlgorithm {
	
	private static final Logger logger = Logger.getLogger(GlycanTreeLayoutAlgorithm.class);

	private double horSpacing = 60;
	private double verSpacing = 40;

	public GlycanTreeLayoutAlgorithm(int styles) {
		super(styles);
	}
	...
	//	int verticalLineSize = (int)(entitiesToLayout[0].getLayoutEntity().getHeightInLayout() + verSpacing);
		
		//int horizontalLineSize = (int)(entitiesToLayout[0].getLayoutEntity().getWidthInLayout() + horSpacing);
		int horizontalLineSize = computeLineSize(entitiesToLayout, true);
		
		// the vertical location we begin placing the nodes from
	//	int heightSoFar = (int) (((rows.size()-1) * verticalLineSize) + verSpacing);
		
		int widthSoFar = (int) (((rows.size()-1) * horizontalLineSize) + horSpacing);
		
		// place bottom most row first - just center it.
		List<GXMoreThanNode> childRow = rows.get(0);
		//int nodeSpacing = (int) (verSpacing + childRow.get(0).getNode().getLayoutEntity().getHeightInLayout());
		int nodeSpacing = computeLineSize(entitiesToLayout, false);
	//	int y = (int) ((boundsHeight / 2) - ((childRow.size()/2) * nodeSpacing));		
		int y = (int) verSpacing;
		placeStrand(childRow, widthSoFar, y, nodeSpacing);
		...
		for (int i = 1; i < rows.size(); i++) {
			...
			widthSoFar -= horizontalLineSize;
			placeRow(parentRow, widthSoFar);
		}
		
		logger.debug("Finished applying layout");
	}
	
	private int computeLineSize (InternalNode[] entitiesToLayout, boolean horizontal) {
		int lineSize = (int) (horizontal ? horSpacing : verSpacing);
		for (InternalNode internalNode : entitiesToLayout) {
			lineSize = Math.max(lineSize, (int)(horizontal? internalNode.getWidthInLayout() + horSpacing : internalNode.getHeightInLayout() + verSpacing));
		}
		return lineSize;
	}
	 ...
	 */
	private void placeRow(List<GXMoreThanNode> row, int xLocation) {
		...
		for (int j = 0; j<row.size(); j++) {
			...
			if (!parentRight.getChildren().isEmpty()) {
				...
				parentY /= parentRight.getChildren().size();
				parentRight.setLocation(xLocation, parentY);
				
				// and layout the childless strand
				if (!childlessStrand.isEmpty()) {
					placeChildless(childlessStrand, parentLeft, parentRight, xLocation);
		...
		if (!childlessStrand.isEmpty()) {
			placeChildless(childlessStrand, parentLeft, null, xLocation);			
		}
	}

	/**
	 * Builds the matrix of nodes. each node will be wrapped with necessary information
	...
	private void placeChildless(List<GXMoreThanNode> childlessStrand,
			GXMoreThanNode parentLeft, GXMoreThanNode parentRight, int xLoc) {
		int startMark = 0;
		int spacing = (int)(childlessStrand.get(0).getNode().getLayoutEntity().getHeightInLayout() + verSpacing);
		
		// There's only a parent on the right
		if (parentLeft==null  && parentRight!=null) {
			startMark = parentRight.getY() - (spacing *childlessStrand.size()); 
		} // there's a parent on the left 
		else if ( parentLeft!=null) {
			startMark = parentLeft.getY() + spacing; 
			...
			if (parentRight != null) {
				...
				if (endMark > parentRight.getY()) {
					// shift everything on the right to the right by the missing amount of space.
					shiftTreesRightOfMark(parentRight, endMark - parentRight.getY());
				}
			}
		}
		
		// now the room has been assured, place strand.
		placeStrand(childlessStrand, xLoc , startMark, spacing);			
	}
	...
	private void shiftTreesRightOfMark(GXMoreThanNode mark, int shift) {
		mark.setLocation(mark.getX(), mark.getY() + shift);
		GXMoreThanNode leftMostChild = getTopMostChild(mark);
	 ...
	 */
	private GXMoreThanNode getTopMostChild(GXMoreThanNode parent) {
		GXMoreThanNode topMost = parent.getChildren().get(0);

		// run through children 
		for (GXMoreThanNode child:parent.getChildren()) {
			if (child.getY() < topMost.getY()) {
				topMost = child;
			}
		}
		return topMost;
	...
	private void shiftTree(GXMoreThanNode root, int shift) {
		root.setLocation(root.getX(), root.getY() + shift);
	...
	private InternalNode getFirstEntity(List<InternalNode> entitiesList, List<InternalRelationship> relationshipsList) {
		...
		if (!entitiesLeft.isEmpty()) {
			// find a node with some children
			boolean hasRelationship=false;
			for (InternalNode internalNode : entitiesLeft) {
				for (InternalRelationship rel:relationshipsList) {
					if (rel.getSource() == internalNode)  { // that means this node has children
